---
title: 'Notes de Leçon | Travailler avec les Chaînes de Caractères en R'
output:
  html_document:
...
```{r, eval = FALSE}
will_not_work <- "Guillemets doubles " à l'intérieur de guillemets doubles"
will_not_work <- 'Guillemets simples ' à l'intérieur de guillemets simples'
```
...
```{r}
single_inside_double <- "Guillemets simples ' à l'intérieur de guillemets doubles"
```
...
```{r}
single_quote <- 'Guillemets simples \' à l'intérieur de guillemets doubles'
double_quote <- "Guillemets doubles \" à l'intérieur de guillemets doubles"
```

Pour afficher ces chaînes telles qu'elles apparaîtraient dans la sortie, comme sur un graphique, utilisez `cat()` :

```{r}
cat('Guillemets simples \' à l'intérieur de guillemets doubles')
cat("Guillemets doubles \" à l'intérieur de guillemets doubles")
```
...
```{r}
lima_messy_1 <- openxlsx::

read.xlsx(here("data/lima_messy_1.xlsx")) %>% 
  as_tibble()
lima_messy_1
```
